...
backup_url_address = 'https://node.somenano.com/proxy'
...
def get_pow(hash):
    ...
    if 'work' in resulting_data:
        ...
        data = requests.post(backup_url_address, json = {"action" : "work_generate", "hash" : hash, "multiplier" : "1.0"}, timeout=30)
        #Generate work
        resulting_data = data.json()
        if 'work' in resulting_data:
            work = resulting_data['work']
            print('Backup work: {}'.format(work))
            return work
